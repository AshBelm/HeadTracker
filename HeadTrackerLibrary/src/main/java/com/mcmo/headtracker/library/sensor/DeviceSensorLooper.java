...
public class DeviceSensorLooper implements SensorEventProvider{
    ...
    private Sensor getUncalibratedGyro() {
        return Build.MANUFACTURER.equals("HTC")?null:this.sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE_UNCALIBRATED);//获取一个未校准陀螺仪传感器，提供原始的、未校准、补偿的陀螺仪数据，用于后期处理和融合定位数据。
    }
        ...
        if(!this.isRunning) {
            ...
            HandlerThread sensorThread = new HandlerThread("sensor") {
                protected void onLooperPrepared() {
                    ...
                    Sensor accelerometer = DeviceSensorLooper.this.sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);
                    ...
                    if(gyroscope == null) {
                        ...
                        gyroscope = DeviceSensorLooper.this.sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE);
                    }
