...
import com.mcmo.headtracker.library.sensor.internal.GyroscopeBiasEstimator;
import com.mcmo.headtracker.library.sensor.internal.Matrix3x3d;
import com.mcmo.headtracker.library.sensor.internal.OrientationEKF;
import com.mcmo.headtracker.library.sensor.internal.Vector3d;
...
public class HeadTracker implements SensorEventListener{
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
        ...
        } else if(event.sensor.getType() == Sensor.TYPE_GYROSCOPE || event.sensor.getType() == Sensor.TYPE_GYROSCOPE_UNCALIBRATED) {
            ...
            if(event.sensor.getType() == Sensor.TYPE_GYROSCOPE_UNCALIBRATED) {
                ...
                //The uncalibrated gyroscope is similar to the gyroscope, except that no gyro-drift compensation is applied to the rate of rotation
                //The uncalibrated gyroscope is useful for post-processing and melding orientation data. In general, gyroscope_event.values[0] will be close to uncalibrated_gyroscope_event.values[0] - uncalibrated_gyroscope_event.values[3]. That is,
